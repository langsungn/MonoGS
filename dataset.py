...
class TelloDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        self.has_depth = False
        self.num_imgs = 300
        dataset_path = config["Dataset"]["dataset_path"]
        parser = Telloparser(dataset_path)
        self.color_paths = sorted(glob.glob(f"{dataset_path}rgb/*.png"))
        print("number of color_path", len(self.color_paths))
...
def load_dataset(args, path, config):
    ...
    elif config["Dataset"]["type"] == "tello":
        return TelloDataset(args, path, config)   
    else:
